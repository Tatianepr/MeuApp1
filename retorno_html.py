...
def home():
    session = Session()
    lista = session.query(Produto).order_by(Produto.id)

    return render_template("home.html", produtos=lista), 200
...
def del_produto(produto_id):
    ...
    # comentario = session.query(Comentario).filter(Produto.id == produto_id).delete()
    # session.commit()

    count = session.query(Produto).filter(Produto.id == produto_id).delete()
    session.commit()

    quant = session.query(Comentario).filter(Comentario.produto == produto_id).delete()
    session.commit()

    if count ==1 or quant==1:
...
def add_comentario(produto_id):
    ...
    session.commit()
    return render_template("produto.html", produto=produto, comentario=comentario ), 200